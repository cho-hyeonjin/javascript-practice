...
▲ 이해를 돕기 위해 레지스트리 개념에 대한 그림을 그려 첨부합니다.
...
좋은 아이디어지만 지금 만드는 애플리케이션은 '순수 함수'를 이용해야 합니다.

하지만 상속을 받으려면 함수가 생성자 함수 형태로 만들어져야 함을 의미하는데, **순수 함수는 생성자 함수로 만들 수 없습니다.**

순수함수란 오직 매개변수를 통해서만 input 값을 받고, 그 값에 대한 output 값을 리턴하기만 하는 함수입니다.

그럼 어떻게 해야 할까요?

상속 대신 `고차함수를 이용하여 동일한 함수 시그니처를 공유` 하는 방식을 사용할 수 있겠습니다.

컴포넌트 메서드를 고차 함수로 래핑하는 것이죠!
